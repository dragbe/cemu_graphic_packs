...
'Asm_PPCBranch = Asm_PPCBranch + (((lngTargetAddress - lngInstructionAddress) \ 4) And &HFFFFFF) * 4
    Asm_PPCBranch = &H48000000 + IIf(blnLr, 1, 0)
...
Private Function Asm_PPCAsmBranchTarget(ByRef ASM_BRANCH_JUMP As ASM_BRANCH_JUMPS, ByVal lngTo As Long, Optional ByRef lngFrom As Long = 0) As String
    ...
    Asm_PPCAsmBranchTarget = IIf(lngTo < 0, "-0x" + Hex(Abs(lngTo)), "+0x" + Hex(lngTo))
End Function
...
'Usage example with the immediate window for Strncmp28: ?Asm_PPCFastStrcmpx(28,true,&h2A376DC,"D:\WiiU\default\dump\U-King.asm")
    ...
    strSnippetVars(0) = IIf(blnStrncmp, "n", "")
...
Dim strAsm As String
    strSnippetVars(0) = IIf(intStrptrOffsetL1 < 0, "-0x" + Hex(Abs(intStrptrOffsetL1)), "0x" + Hex(intStrptrOffsetL1))
    strSnippetVars(1) = IIf(intStrptrOffsetL2 < 0, "-0x" + Hex(Abs(intStrptrOffsetL2)), "0x" + Hex(intStrptrOffsetL2))
    If btCharsCount < ASM_GPRSCOUNT Then
...
'Usage example with the immediate window for Strcmp call: ?Asm_PPCFastStrlenCall(&h2A376DC,&h8,6,30,"D:\WiiU\default\dump\U-King.asm")
    strSnippetVars(0) = IIf(intStrptrOffsetL < 0, "-0x" + Hex(Abs(intStrptrOffsetL)), "0x" + Hex(intStrptrOffsetL))
    strSnippetVars(1) = ""
...
'Usage example with the immediate window: ?Asm_PPCStrpbrkCall(0,4,&h10,1,22,&h2B75B54,"D:\WiiU\default\dump\U-King.asm")
    strSnippetVars(1) = IIf(intStrptrOffsetL < 0, "-0x" + Hex(Abs(intStrptrOffsetL)), "0x" + Hex(intStrptrOffsetL))
    strSnippetVars(0) = IIf(intSearchedCharsOffsetL < 0, "-0x" + Hex(Abs(intSearchedCharsOffsetL)), "0x" + Hex(intSearchedCharsOffsetL))
