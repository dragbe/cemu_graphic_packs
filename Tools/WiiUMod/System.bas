...
End Function
Public Function System_ShellAndWait(ByVal intExitOnError As Integer, ByVal lngWindowsStyle As Long, ParamArray varCommands() As Variant) As Long
'lngWindowsStyle: 0 (Hide), 3 (MaximizedFocus), 2 (WshMinimizedFocus), 6 (MinimizedNoFocus), 1 (NormalFocus), 4 (NormalNoFocus)
'varCommands: shell commands in reverse order (UBound(varCommands)<=30)
Dim objWsh As Object
Dim strTempFilePath As String
Dim strBatchFilePath As String
    Set objWsh = CreateObject("WScript.Shell")
    If intExitOnError = 0 Then
        intExitOnError = UBound(varCommands)
        If intExitOnError = 0 Then
            If objWsh.Run(CStr(varCommands(intExitOnError)), lngWindowsStyle, True) <> 0 Then
                System_ShellAndWait = System_ShellAndWait + 2& ^ intExitOnError
            End If
        Else
            System_ShellAndWait = FreeFile
            strTempFilePath = File_GetTempFilePath
            Open strTempFilePath For Output As System_ShellAndWait
            Print #System_ShellAndWait, "@ECHO OFF"
            Do
                Print #System_ShellAndWait, CStr(varCommands(intExitOnError))
                intExitOnError = intExitOnError - 1
            Loop Until intExitOnError < 0
            Print #System_ShellAndWait, "EXIT"
            Close System_ShellAndWait
            strBatchFilePath = strTempFilePath + ".bat"
            Name strTempFilePath As strBatchFilePath
            System_ShellAndWait = objWsh.Run(strBatchFilePath, lngWindowsStyle, True)
            Kill strBatchFilePath
        End If
    Else
        For intExitOnError = UBound(varCommands) To 0 Step -1
            If objWsh.Run(CStr(varCommands(intExitOnError)), lngWindowsStyle, True) <> 0 Then
                System_ShellAndWait = intExitOnError
                Exit For
            End If
        Next intExitOnError
    End If
